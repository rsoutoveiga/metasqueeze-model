###############################################################################
# Visualisation of the results of Experiment 2 using the MetaSqueeze model in 
...
# input files
g_path <- "data/sim_outputs/experiments/02_experiment"
g_sim_type <- "*metapop.csv"

# output figure
g_figure_name <- "02_experiment"
plot_text <- ""
...
g_params <- g_params[, c("sim_id", "val1", "val2")]

# rename columns
base::colnames(g_params) <- c("sim_id", "fire_interval", "mort")
...
g_df_summ$mort <- base::as.factor(g_df_summ$mort)

# calculate persistence difference between mortality scenarios
g_df_mort_2 <- g_df_summ %>% 
  dplyr::filter(mort == 2)

g_df_mort_5 <- g_df_summ %>% 
  dplyr::filter(mort == 5)

g_df_mort_5$difference <- g_df_mort_5$grand_mean_year_max - g_df_mort_2$grand_mean_year_max

base::min(g_df_mort_5$difference)
base::max(g_df_mort_5$difference)
...
p1 <- ggplot2::ggplot(data = g_df_summ) +
  ...
  ggplot2::scale_y_continuous(
    breaks = base::seq(from = 50, to = 150, by = 25)
  ) +
  ggplot2::scale_x_continuous(
    breaks = base::seq(from = 5, to = 35, by = 5)
  ) +
  ggplot2::coord_cartesian(ylim = c(40, 150)) +
  scale_color_manual(
    name = "Mortality scenario",
    labels = c("past competition", "lower competition"),
