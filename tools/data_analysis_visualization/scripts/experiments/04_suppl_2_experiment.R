###############################################################################
# Visualisation of the results of Experiment 4 (Supplementary 2) using the 
...
g_folder_1 <- "/04_02_experiment"
g_folder_2 <- "/04_suppl_2_01_experiment"
g_folder_3 <- "/04_suppl_2_02_experiment"
g_folder_4 <- "/04_suppl_2_03_experiment"

# simulation output type
g_sim_type <- "*metapop.csv"

# name output figure
g_figure_name <- "04_supple_2_experiment"

g_df_all <- base::data.frame()
g_folders <- c(g_folder_1, g_folder_2, g_folder_3, g_folder_4)
...
for (i in g_folders) {
  ...
  g_df_summ <- g_df %>%
    ...
    dplyr::summarise(
      ...
      mean_occu_mean = base::mean(occu_mean, na.rm = TRUE),
      sd_occu_mean = stats::sd(occu_mean, na.rm = TRUE)
    )
  ...
  g_df_summ <- g_df_summ %>%
    ...
    dplyr::summarise(
      ...
      # occupancy
      grand_mean_occu_mean = base::mean(mean_occu_mean, na.rm = TRUE),
      grand_sd_occu_mean = fun_pooled_sd_equal_sample_sizes(sd_occu_mean),
      plus_sd_occu_mean = dplyr::if_else(
        (grand_mean_occu_mean + grand_sd_occu_mean) > 100,
        100,
        grand_mean_occu_mean + grand_sd_occu_mean
      ),
      minus_sd_occu_mean = dplyr::if_else(
        (grand_mean_occu_mean - grand_sd_occu_mean) < 0,
        0,
        grand_mean_occu_mean - grand_sd_occu_mean
      )
    )
...
p1 <- ggplot2::ggplot(data = g_df_all) +
  ...
  ggplot2::geom_ribbon(
    mapping = aes(
      ...
      color = base::as.factor(sort_size)
      ),
    size = 0.3,
    alpha = 0.13,
    linetype = 2
  ...
  ggtitle("(a)") +
  ggplot2::xlab("# of dunes with increased \nquality (high or moderate)") +
  ...
  scale_color_manual(
    ...
    labels = c("one high", "two high", "three high", "all high"),
    values = c("#868686FF", "#009E73", "#0073C2FF", "#EFC000FF")
  ) +
  scale_fill_manual(
    name = "Maximum # of high\n    quality dunes",
    breaks = c(
...
p2 <- ggplot2::ggplot(data = g_df_all) +
  ggplot2::geom_line(
    mapping = aes(
      x = num_pops,
      y = grand_mean_occu_mean,
      group = base::as.factor(sort_size), 
      colour = base::as.factor(sort_size)),
    size = 0.8) + 
  ggplot2::geom_ribbon(
    mapping = aes(
      x = num_pops,
      ymin = minus_sd_occu_mean,
      ymax = plus_sd_occu_mean,
      group = base::as.factor(sort_size), 
      fill = base::as.factor(sort_size),
      color = base::as.factor(sort_size)
      ),
  ...
  ggtitle("(b)") +
  ggplot2::xlab("# of dunes with increased \nquality (high or moderate)") +
  ggplot2::ylab("Dune occupancy (%)") +
  ggplot2::scale_y_continuous(
    breaks = base::seq(from = 0, to = 100, by = 10)
  ) +
  ggplot2::scale_x_continuous(
    breaks = base::seq(from = 0, to = 37, by = 5)
  ) +
  ggplot2::coord_cartesian(ylim = c(20, 65)) +
  scale_color_manual(
    name = "Maximum # of high\n    quality dunes",
    breaks = c(
      "/05_01_experiment",
    ...
    # labels = c("all high", "three high", "two high", "one high"),
    # values = c("#0073C2FF", "#009E73", "#EFC000FF", "#868686FF")
    labels = c("one high", "two high", "three high", "all high"),
    values = c("#868686FF", "#009E73", "#0073C2FF", "#EFC000FF")
  ) +
  scale_fill_manual(
...
ggplot2::ggsave(
  plot = g_parr,
  filename = base::paste0(g_figure_name, ".tiff"),
...
ggplot2::ggsave(
  ...
  path = base::paste0(g_root_path, g_folder_3, "/plots"),
  width = 3.31 * 2,
  height = 4,
  bg = "white"
)
...
ggplot2::ggsave(
  plot = g_parr,
  filename = base::paste0(g_figure_name, ".png"),
  path = base::paste0(g_root_path, g_folder_4, "/plots"),
  width = 3.31 * 2,
  height = 4,
  bg = "white"
)
