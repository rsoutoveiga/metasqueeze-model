###############################################################################
# Visualisation of the results of Experiment 2 (Suppl.) using the MetaSqueeze 
...
# input files
g_path <- "data/sim_outputs/experiments/02_suppl_experiment"
g_sim_type <- "*metapop.csv"

# output figure
g_figure_name <- "02_test_experiment"
plot_text <- ""
...
p1 <- ggplot2::ggplot(data = g_df_summ) +
  ...
  ggplot2::scale_x_continuous(
    breaks = base::seq(from = 5, to = 30, by = 5)
  ) +
  scale_color_manual(
    name = "Mortality scenario",
    labels = c("relative", "absolute", "lower competition"),
